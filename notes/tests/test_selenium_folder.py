...
class TestFolder(LoginTest):
    ...
    """
    Makes sure that the user is able to open an existing folder
    """
    def test_open_existing_folder(self):
        ...
        sleep(1)
        self.assertEqual('http://localhost:8000/notes/81/', self.driver.current_url)
    

    """
    Makes sure that the user is able to create a new folder
    """
    ...
    """
    Makes sure that the user is able to rename a folder
    """
    def test_rename_folder(self):
        self.driver.get(self.shared_data['new_folder_url'])
        self.driver.find_element(By.ID, 'settingsButton').click()
        sleep(0.5)
        self.driver.find_element(By.ID, 'newFolderTitle').send_keys('New Folder')
        self.driver.find_element(By.ID, 'renameFolderButton').click()
        sleep(2)
        self.driver.get(self.shared_data['new_folder_url'])
        sleep(0.5)
        self.assertIn('New Folder', self.driver.find_element(By.ID, 'folderTitle').text)
    ...
    def test_trash_folder(self):
        ...
        sleep(0.5)
        self.assertEqual('http://localhost:8000/notes/80/', self.driver.current_url)